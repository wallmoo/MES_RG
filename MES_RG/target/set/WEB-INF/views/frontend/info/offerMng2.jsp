...
function init_grid_list(){
	...
	$('#grid_list').w2grid({ 
        ...
        columns: [                
			{ field:'order_num', caption:'주문번호', size:'10%', style:'text-align:center'},
			{ field:'request_num', caption:'발주번호', size:'10%', style:'text-align:center'},// 200px
			{ field:'customer_nm', caption:'고객사명', size:'10%', style:'text-align:center'},
			{ field:'manager_nm', caption:'담당자명', size:'7%', style:'text-align:center'},// 120px
			{ field:'land', caption:'사업장', size:'10%', style:'text-align:center'}, 
			{ field:'item_code', caption:'P/N', size:'10%', style:'text-align:center'},// 120px
			...
			{ field:'serial_no', caption:'S/N', size:'10%', style:'text-align:center' },
			{ field:'unit_price', caption:'출고단가', size:'10%', style:'text-align:center'
			...
			{ field:'menge', caption:'주문수량', size:'8%', style:'text-align:center'},
			{ field:'total_price', caption:'전체금액(원)', size:'10%', style:'text-align:center'
				,render: function (record, index, col_index) {
	       	        var html = this.getCellValue(index, col_index);
			...
			{ field:'delivery_date', caption:'주문일자', size:'10%', style:'text-align:center'},
			{ field:'comment', caption:'특이사항', size:'20%', style:'text-align:center'},
			{ field:'fast_yn', caption:'긴급', size:'8%', style:'text-align:center'
				,render: function (record, index, col_index) {
					var html = this.getCellValue(index, col_index);
...
function init_grid_list2(){
	...
	$('#grid_list2').w2grid({ 
        ...
        columns: [                
			...
			{ field:'', caption:'', size:'10%', style:'text-align:center'},
			{ field:'', caption:'', size:'20%', style:'text-align:center'},
			{ field:'', caption:'', size:'8%', style:'text-align:center'}
...
function modalGrid() {
	...
	$('#modalGrid').w2grid({ 
        ...
        columns: [              
			...
			{ field:'fast_yn', caption:'긴급', size:'8%',  style:'text-align:center', editable: { type: 'checkbox' }}
...
function saveProcess()
{
	...
	$.each(records, function(idx, row){
		...
		if(row.item_code != '' && row.item_code != 'null' && row.item_code != null){
			...
			saveItem(row,order_num, row.recid, fast_yn);
		}
...
//저장
function saveItem(row,order_num, recid, fast_yn) {
		...
		success : function( data ) {
			if(data.status == 200 && (data.rows).length>0 ) {
				...
				$.each(rowArr, function(idx, row){
					...
					if(row.customer_nm)comboValue_nm3.push(row.customer_nm);
					if(row.request_num)comboValue_nm4.push(row.request_num);
				});
		...
		success : function( data ) {
			if(data.status == 200 && (data.rows).length>0 ) {
				...
				$.each(rowArr, function(idx, row){
					...
					//row.fast_yn = '<input type="radio" name="rdo_fast_yn" value="N">NO <input type="radio" name="rdo_fast_yn" value="Y">YES';
...
function excelFileDownload()
{
	...
	var fileName = '주문등록/취소.xlsx';
	var sheetTitle = '주문등록/취소';
	var sheetName = '주문등록/취소';
...
function add_register()
{
	...
	if( grid3Arr.length > 0  ) {	
		...
		for(var i = 0; i < grid3Arr.length; i ++)
		{
			...
			//fast_yn = '<input type="radio" name="rdo_fast_yn" value="N" checked>NO <input type="radio" name="rdo_fast_yn" value="Y">YES';
			...
			w2ui['modalGrid'].add([
				               		{ recid:  currentRecid, item_code: data.item_code , item_nm: data.item_nm, menge: menge, unit_price : release_price, total_price : total_price, fast_yn : fast_yn}
				               	]);

...
<div class="wrapper">
 ...
 <div class="content-wrapper">
    <section class="content-header">
      ...
      <ol class="breadcrumb">
        <li><a href="#"><i class="fa fa-dashboard"></i>품질관리</a></li><li class="active">공정검사</li>
      </ol>
    </section>
...
<!-- 							</div>	 -->
							...
							<div class="col-sm-4">
								<div class="form-group" style="margin-bottom: 0px;">
									<label>공정명</label> 
									<div>
										<input type="text" id="m_routing_nm" name="m_routing_nm" placeholder="ex) 공정명"
										  class="form-control input-sm cond_1" />
									</div>
								</div>
							</div>	
<!-- 							<div class="col-sm-4"> -->
...
$(function($) {
 	...
 	$(':radio[name="rdo_insp_case"]').on('ifChecked', function(event){
 		var han_sample_type = $('#p_sample_type').html();
 		var sample_type = '';
 		if(han_sample_type == '초물') sample_type = 'a';
 		if(han_sample_type == '중물') sample_type = 'b';
 		if(han_sample_type == '종물') sample_type = 'c';
 		
 		w2ui['grid_inpt'].destroy();
 		init_grid_inpt();
 		loadInpt(sample_type);
 	}); 
...
function init_grid_chk(){
	...
	$('#grid_chk').w2grid({ 
        ...
        columns: [                
			...
			{ field:'min_spec', caption:'규격(LSL)', size:'10%', style:'text-align:center', hidden:true},
			{ field:'max_spec', caption:'규격(USL)', size:'10%', style:'text-align:center', hidden:true},
...
function loadList()
{
    ...
    for(var i =0; i<$('#m_routing_nm').data('selected').length; i++) {
       
       if(i==0){
          m_combo =  $('#m_routing_nm').data('selected')[i].id;
       } else {
          m_combo = m_combo + "," + $('#m_routing_nm').data('selected')[i].id;
       }
    }
    console.log($('#m_routing_nm').data('selected').length);
	}
    console.log(m_combo);
    
 	var routing_nm = $('#m_routing_nm').data('selected');
	...
	var postData = "product_po=" + encodeURIComponent($("#m_product_po").val())
			...
			+ "&routing_nm=" + encodeURIComponent(m_combo)
	...
	$.ajax({
		...
		success : function(data) {
			if (data.status == 200 && (data.rows).length > 0) {
				...
				$.each(rowArr, function(idx, row) {
					...
					if(row.routing_nm)comboValue_nm4.push(row.routing_nm);
				});
				...
				if (startValue_combo == "") {
					...
					$('#m_routing_nm').w2field('enum', { items: _.uniq(comboValue_nm4,false) ,match : 'contains' });
				}
		...
		complete : function() {
			...
			document.getElementById("m_routing_nm").style.removeProperty("height");


			console.log("m_item_nm:"+$('#m_item_nm'));
...
// $('#m_routing_nm').w2field('m_routing_nm', { 
//     items: rowArr, 
//     openOnFocus: true
...
// 				row.w2ui = { "style": "text-decoration: line-through" };
				row.w2ui = { "style": "text-decoration: line-through;-webkit-text-decoration-color: red; text-decoration-color: red;" };
			}

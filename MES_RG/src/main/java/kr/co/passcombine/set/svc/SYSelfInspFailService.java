...
import kr.co.passcombine.set.dao.SYSelf_InspFailDAO;
...
@Service(value = "setSelfInspFailService")
@Transactional(propagation = Propagation.REQUIRED, rollbackFor = { Exception.class })
public class SYSelfInspFailService {

	@Resource
	private Environment environment;
	
	@Resource(name = "setSelf_InspFailDAO")
	private SYSelf_InspFailDAO dao;


	public List<SYSelf_InspFailVo> selectSelfInspFail(SYSelf_InspFailVo vo) {
		return dao.selectSelfInspFail(vo);
	}
	
	
	public int saveSelfInspFail(SYSelf_InspFailVo vo){
		return dao.saveSelfInspFail(vo);
	}
	
	public int deleteSelfInspFail(SYSelf_InspFailVo vo){
		return dao.deleteSelfInspFail(vo);
	}
	
	public int updateSelfInspFail(SYSelf_InspFailVo vo){
		return dao.updateSelfInspFail(vo);
	}
	
	public int saveSelfInspFail2(SYSelf_InspFailVo vo){
		return dao.saveSelfInspFail2(vo);
	}

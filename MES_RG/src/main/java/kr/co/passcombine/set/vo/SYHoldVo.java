package kr.co.passcombine.set.vo;

public class SYHoldVo {
	private String product_po;
	private String hold_seq;
	private String routing_code;
	private String routing_seq;
	private String real_seq;
	private String prev_status;
	private String machine_code;
	private String machine_nm;
	private String prod_str_time;
	private String hold_str_time;
	private String hold_comment;
	private String del_yn;
	
	private String creator;
	private String updater;
	private String date_created;
	private String date_updated;
	
	
	
	
	
	public String getPrev_status() {
		return prev_status;
	}
	public void setPrev_status(String prev_status) {
		this.prev_status = prev_status;
	}
	public String getProduct_po() {
		return product_po;
	}
	public void setProduct_po(String product_po) {
		this.product_po = product_po;
	}
	public String getHold_seq() {
		return hold_seq;
	}
	public void setHold_seq(String hold_seq) {
		this.hold_seq = hold_seq;
	}
	public String getRouting_code() {
		return routing_code;
	}
	public void setRouting_code(String routing_code) {
		this.routing_code = routing_code;
	}
	public String getRouting_seq() {
		return routing_seq;
	}
	public void setRouting_seq(String routing_seq) {
		this.routing_seq = routing_seq;
	}
	public String getReal_seq() {
		return real_seq;
	}
	public void setReal_seq(String real_seq) {
		this.real_seq = real_seq;
	}
	public String getMachine_code() {
		return machine_code;
	}
	public void setMachine_code(String machine_code) {
		this.machine_code = machine_code;
	}
	public String getMachine_nm() {
		return machine_nm;
	}
	public void setMachine_nm(String machine_nm) {
		this.machine_nm = machine_nm;
	}
	public String getProd_str_time() {
		return prod_str_time;
	}
	public void setProd_str_time(String prod_str_time) {
		this.prod_str_time = prod_str_time;
	}
	public String getHold_str_time() {
		return hold_str_time;
	}
	public void setHold_str_time(String hold_str_time) {
		this.hold_str_time = hold_str_time;
	}
	public String getHold_comment() {
		return hold_comment;
	}
	public void setHold_comment(String hold_comment) {
		this.hold_comment = hold_comment;
	}
	public String getDel_yn() {
		return del_yn;
	}
	public void setDel_yn(String del_yn) {
		this.del_yn = del_yn;
	}
	public String getCreator() {
		return creator;
	}
	public void setCreator(String creator) {
		this.creator = creator;
	}
	public String getUpdater() {
		return updater;
	}
	public void setUpdater(String updater) {
		this.updater = updater;
	}
	public String getDate_created() {
		return date_created;
	}
	public void setDate_created(String date_created) {
		this.date_created = date_created;
	}
	public String getDate_updated() {
		return date_updated;
	}
	public void setDate_updated(String date_updated) {
		this.date_updated = date_updated;
	}
	
	
	
	
	
}
...
<div class="wrapper">
	...
	<div class="content-wrapper">
			<section class="content-header">
				<h1>
					주문현황 <small>주문/출하관리</small>
				</h1>
				<ol class="breadcrumb">
					<li><a href="#"><i class="fa fa-dashboard"></i> 주문현황</a></li>
...
<!-- 								<button type="button" id="btn_search" class="btn btn-danger btn-sm"  -->
<!-- 								onclick="deleteItem2();">삭제2</button>	 -->
								...
								<div class="col-sm-2">
									<div class="form-group" style="margin-bottom: 0px;">
										...
										<select id="process_status" class="form-control select2 input-sm" data-placeholder="" style="width: 100%;" onChange = "loadList();">
												...
												<option value="B">출고전</option>
												<option value="A">출고후</option>
...
function init_grid_list(){
	...
	$('#grid_list').w2grid({ 
        ...
        columns: [      
			...
			{ field:'process_status', caption:'상태', size:'10%', style:'text-align:center',sortable :true
...
function deleteItem2() {
	...
	//loadListOther(order_num, item_code);
	...
	if( key.length==0 ) {
		...
		fnMessageModalConfirm("알림", "선택한 내용을 삭제하시겠습니까?", function(result) {
			if(result) {
				...
				var strUrl = "/frontend/consumable/deleteOrder2";
					 ...
					 success:function(data, textStatus, jqXHR){
					 	if(data.status == "200") {
					    	...
					    	fnMessageModalAlert("결과", "정상적으로 처리되었습니다."); // Notification(MES)
					 		...
					 		fnMessageModalAlert("결과", "이미 작업지시가 생성된 작업입니다.");
					 	}
					 },
					 error: function(jqXHR, textStatus, errorThrown){
					    	fnMessageModalAlert("결과", "정보를 처리하는데 에러가 발생하였습니다.");	 // Notification(MES)
...
function loadList() {
	...
	var page_url = "/frontend/consumable/selectOrderAll";
...
function loadListOther(order_num, item_code) {

	console.log("loadListOther()");
	...
	$.ajax({
		...
		success : function( data ) {
			...
			if(rowArr.length > 0){
				fnMessageModalAlert("알림", "이미 작업지시 생성된 주문입니다."); // Notification
				return;
			}else{
				deleteItem2();
			}

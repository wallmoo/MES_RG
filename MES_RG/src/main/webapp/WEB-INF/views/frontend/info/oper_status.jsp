					    ...
					    <iframe id="iframeLoading" frameborder="0" style="z-index:-1; position:absolute; visibility:hidden"></iframe>
							...
							<div id="" class="box-body">
								...
								<div class="form-group">
									...
									<div class="col-sm-2">
										...
										<div class="input-group">
											...
											<input type="combo" id="work_end_time" name="work_end_time"
									...
									<div class="col-sm-2">
										<div class="form-group">
											<label>설비코드</label> <select id="machine_nm" name="machine_nm"
									...
									<div class="col-sm-2">
										<div class="form-group" style="margin-bottom: 0px;">
											<label>EVENT</label>
											<div class="form-group" style="margin-bottom: 0px;">
												<select id="gb" class="form-control select2 input-sm"
								...
								<div class="form-group">
									...
									<div class="col-md-12">
										<div id="grid_list2"
											style="width: 100%; height: 60px; margin-top: 20px;"></div>
									</div>
	...
	loadList2();
					...
					.ajax({
						...
						success : function(data) {
							if (data.status == 200 && (data.rows).length > 0) {
								...
								$
										.each(
												...
												function(idx, row) {
													...
													comboValue_ic
															.push(row.item_code ? row.item_code
																	+ ''
																	: '');
													comboValue_mc
															.push(row.item_nm ? row.item_nm
																	+ ''
																	: '');

													var date_split = row.work_end_time
															.split(" ");
													var grap_date = date_split[0];
													var next_grap_date = '';

													try {
														next_grap_date = rowArr[Number(idx) + 1].work_end_time
																.split(" ")[0];
														console
																.log('grap_date : '
																		+ grap_date);
													...
													if (row.gb == 'PROCESS') {
														grap_process = Number(grap_process)
																+ Number(work_time
																		.replace(
																				"시간",
																				""));
													...
													if (row.gb == 'UNPROCESS') {
														...
														grap_unprocess = Number(grap_unprocess)
																+ Number(work_time
																		.replace(
																				"시간",
																				""));
													...
													if (row.gb == 'WAITING') {
														...
														grap_waiting = Number(grap_waiting)
																+ Number(work_time
																		.replace(
																				"시간",
																				""));
													...
													var hap = Number(grap_process)
															+ Number(grap_unprocess)
															+ Number(grap_waiting);
													console.log("hap??" + hap);
													console
															.log("grap_process??"
													...
													grap_process_per = grap_process_per
															.toFixed(1);

													var grap_unprocess_per = (Number(grap_unprocess) / Number(hap)) * 100;
													grap_unprocess_per = grap_unprocess_per
															.toFixed(1);

													var grap_waiting_per = (Number(grap_waiting) / Number(hap)) * 100;
													grap_waiting_per = grap_waiting_per
															.toFixed(1);

													if (grap_process_per == 'NaN') {
														grap_process_per = 0;
													}
													if (grap_unprocess_per == 'NaN') {
														grap_unprocess_per = 0;
													}
													if (grap_waiting_per == 'NaN') {
														grap_waiting_per = 0;
													}

													console
															.log("grap_process_per??"
																	+ grap_process_per);
													console
															.log("grap_unprocess_per??"
																	+ grap_unprocess_per);
													console
															.log("grap_waiting_per??"
																	+ grap_waiting_per);
								...
								process_time_hour = process_time_hour.replace(
										"시간", "");
								unprocess_time_hour = unprocess_time_hour
										.replace("시간", "");
								waiting_time_hour = waiting_time_hour.replace(
										"시간", "");

								console.log("today_time_hour??"
										+ today_time_hour);
								console.log("process_time_hour??"
										+ process_time_hour);
								console.log("unprocess_time_hour??"
										+ unprocess_time_hour);
								console.log("waiting_time_hour??"
										+ waiting_time_hour);
								...
								var process_time_hour_per = process_time_hour
										+ '시간' + '(' + per + ')';
								var unprocess_time_hour_per = unprocess_time_hour
										+ '시간' + '(' + per2 + ')';
								var waiting_time_hour_per = waiting_time_hour
										+ '시간' + '(' + per3 + ')';

								console.log("process_time_hour_per??"
										+ process_time_hour_per);
								console.log("unprocess_time_hour_per??"
										+ unprocess_time_hour_per);
								console.log("waiting_time_hour_per??"
										+ waiting_time_hour_per);
								...
								$('#item_code').w2field('combo', {
									items : _.uniq(comboValue_ic, false),
									match : 'contains'
								});
								document.getElementById("item_code").style
										.removeProperty("height");
								$('#item_nm').w2field('combo', {
									items : _.uniq(comboValue_mc, false),
									match : 'contains'
								});
		...
		function fnLoadCustomerGrid2() {
			...
			$('#grid_list2').w2grid({
				...
				columns : [ {
					...
					field : 'prod_menge_sum',
					caption : '생산수량',
					size : '15%',
					style : 'text-align:center'
				}, {
					field : 'prod_fail_menge_sum',
					caption : '불량수량',
					size : '15%',
					style : 'text-align:center'
				}, {
					...
					field : 'today_time_hour',
					caption : '총작업시간',
					size : '12%',
					style : 'text-align:center'
				}, {
					field : 'process_time_hour',
					caption : 'PROCESS',
					size : '12%',
					style : 'text-align:center'
				}, {
					field : 'unprocess_time_hour',
					caption : '비가동',
					size : '12%',
					style : 'text-align:center'
				}, {
					field : 'waiting_time_hour',
					caption : 'WAITING',
					size : '12%',
					style : 'text-align:center'
				} ],
		...
		$("#btn_dcode_save")
				.on(
						...
						function(e) {
							...
							if ($.trim($("#d_code_nm").val()).length == 0
									|| $.trim($("#d_code_nm_en").val()).length == 0) {
								fnMessageModalAlert("Notification(Code)",
										"코드명을 입력하세요.(영문코드포함)");
								return;
							}
							...
							$("#d_code_use_yn")
									.val(
											$(
													':radio[name="rdo_dcode_use_yn"]:checked')
													.val());
					...
					if (data.status == "200") {
						if (data.cnt > 0) {
							...
							fnMessageModalAlert("알림",
									"입력하신 고객사 코드는 사용할 수 없습니다.(이미 사용중)");
						} else {
							$("#fg_check_yn").val("Y");
							fnMessageModalAlert("알림",
									"입력하신 고객사 코드는 사용할 수 있습니다.");
						}
					} else {
						fnMessageModalAlert("알림",
								"입력하신 고객사 코드 중복여부를 확인하는데 에러가 발생하였습니다.");
					}
				},
				error : function(jqXHR, textStatus, errorThrown) {
					fnMessageModalAlert("알림",
		...
		function updateCustomer() {
			...
			if (key.length == 1) {
				...
				$("#m_customer_integration")
						.val(data.customer_integration_code).prop("selected",
								true);
		...
		function saveCustomer() {
			...
			var customer_integration_code = $(
					"#m_customer_integration option:selected").val();
			var customer_integration_nm = $(
					"#m_customer_integration option:selected").text();
			...
			if ($("#fg_check_yn").val() == "N") {
				...
				return;
					...
					&& !chkSubmit($("#m_customer_code"), "고객사코드를"))
				return;

			if (/* flag=="I" && */!chkSubmit($("#m_customer_nm"), "고객사명을"))
				return;
			...
			console.log("business code = "
					+ $("#m_business option:selected").val());
			console.log("business name = "
					+ $("#m_business option:selected").text());

			console.log("customer_integration code = "
					+ $("#m_customer_integration option:selected").val());
			console.log("customer_integration name = "
					+ $("#m_customer_integration option:selected").text());

			$("#modal_info").modal('hide');

			var strUrl = "/info/customer/saveCustomer";
			var postData = "flag=" + flag + "&customer_code="
					+ encodeURIComponent(customer_code) + "&customer_nm="
					+ encodeURIComponent(customer_nm) + "&business_code="
					+ encodeURIComponent(business_code) + "&business_nm="
					+ encodeURIComponent(business_nm)
					+ "&customer_integration_code="
					+ encodeURIComponent(customer_integration_code)
					+ "&customer_integration_nm="
					+ encodeURIComponent(customer_integration_nm) + "&land="
					+ encodeURIComponent(land) + "&address="
					+ encodeURIComponent(address) + "&telf="
					+ encodeURIComponent(telf) + "&telfx="
					+ encodeURIComponent(telfx) + "&email_address="
					+ encodeURIComponent(email_address) + "&business_reg_num="
					+ encodeURIComponent(business_reg_num);
		...
		function makeLayout() {
			...
			if ('' == gb) {
				layoutStr = '<rMateChart backgroundColor="#FFFFFF" borderStyle="none">'
						...
						+ '</series>' + '</Column2DChart>' + '</rMateChart>';
			} else if ('PROCESS' == gb) {
				layoutStr = '<rMateChart backgroundColor="#FFFFFF" borderStyle="none">'
						+ '<Options>'
						+ '<Caption text="설비별 가동현황"/>'

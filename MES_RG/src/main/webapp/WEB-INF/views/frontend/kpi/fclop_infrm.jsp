...
<!-- 							</div> -->
						...
						<div class="row">
							<div class="col-md-12">
								<div id="chartHolder3" style="width: 99%; height: 300px;"></div>
							</div>
							
							<div class="col-md-12">
                           		<div id=chartHolder4 style="width: 99%; height: 300px; "></div>
                        	</div>
						</div>
...
// 	});
	...
	function loadList() {
		...
		var page_url = "/frontend/kpi/selectOperation_real";
		
		var postData = "year=" + encodeURIComponent(year)
					+ "&machine_class_code="+ encodeURIComponent(machine_class_code)
					+ "&machine_code="+ encodeURIComponent(machine_code)
					+ "&grapType="+ encodeURIComponent("ONE");
		var result ;
			...
			success : function( data ) {
				if(data.status == 200 && (data.rows).length>0 ) {
					...
					for(var i = 0; i< result.length; i++)
					{
						...
						console.log('grapObj.per  : ' + result[i].per+"%" );
	...
	function loadList2() {
		...
		var postData = "year=" + encodeURIComponent(year)
					...
					+ "&machine_class_code="+ encodeURIComponent(machine_class_code)
					+ "&machine_code="+ encodeURIComponent(machine_code)
					+ "&grapType="+ encodeURIComponent("TWO");
		var result ;
	...
	function makeLayout3() {
		...
		layoutStr3 =
         '<rMateChart backgroundColor="#FFFFFF" borderStyle="none">'
             ...
             +'</Options>'
           +'<Combination3DChart showDataTips="true" itemClickJsFunction="gridData" dataTipDisplayMode="axis" paddingTop="0" >' // dataTipJsFunction="dataTipJsFunc"
           /*
           ...
           */
                  ...
                  +'<series>'
                     ...
                     +'<Column3DSeries labelPosition="outside" yField="per" displayName="월별 가동율" outsideLabelJsFunction="seriesLabelFunc">'
//                         +'<fill>'
...
//                         +'</fill>'
					...
					+'<Line2DSeries radius="6" yField="goal" displayName="목표가동율" itemRenderer="CircleItemRenderer" >'
		                 +'<showDataEffect>'
		                    +'<SeriesInterpolate/>'
		                +'</showDataEffect>'
		               +'<stroke>'
	...
	function makeLayout4() {
		...
		layoutStr4 =
         '<rMateChart backgroundColor="#FFFFFF" borderStyle="none">'
           ...
           */
                  +'<horizontalAxis>'
	                  +'<CategoryAxis categoryField="machine_code" padding="0.2"/>'
	              +'</horizontalAxis>'
	              +'<verticalAxis>'
	                  +'<LinearAxis minimum="0"/>'
	              +'</verticalAxis>'
                  ...
                  +'<series>'
                     ...
                     +'<Column3DSeries labelPosition="outside" yField="per" displayName="장비별 가동율" insideLabelJsFunction="seriesLabelFunc" >'
                        +'<fill>'
                           +'<SolidColor color="#41b2e6"/>'
                        +'</fill>'
                          +'<showDataEffect>'
